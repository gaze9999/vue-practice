...
export const useErpStore = defineStore('erp', () => {
  ...
  const setActiveModule = (moduleId: string) => {
    ...
    console.log('[useErpStore] setActiveModule:', moduleId);
  };
  ...
  const setUserRole = (role: ErpRole) => {
    ...
    console.log('[useErpStore] setUserRole:', role);
  };
  ...
  const getRequiredRoles = (path: string) => {
    ...
    return matched ? matched.roles : void 0; // undefined
  };

  const hasRoleAccess = (roles?: ErpRole[]) => {
    return roles ? roles.includes(currentUser.value.role) : !0; // true
  };
